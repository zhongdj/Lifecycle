...
public class JavaAnonymousInnerClass {
  ...
  public JavaAnonymousInnerClass(String outerClassName, String enclosingMethodName, Type[] enclosingMethodArguments, int innerClassSeq, String
      superClass,
  ...
  private void createBridgeCall(ClassGen cgen) throws ClassNotFoundException {
    ...
    iList.append(end);
    final MethodGen callMethodGen = new MethodGen(4161, new ObjectType("java.lang.Object"), Type.NO_ARGS, new String[] {}, "call", thisClassName,
        iList,
  ...
  private void createCall(ClassGen cgen) throws ClassNotFoundException {
    ...
    final MethodGen callMethodGen = new MethodGen(1, wrappedReturnType, Type.NO_ARGS, new String[] {}, "call", thisClassName, iList, cgen
        .getConstantPool());
    callMethodGen.addException("java.lang.Exception");
    addMethod(cgen, callMethodGen);
    iList.dispose();
  ...
  private boolean argumentsMatch(Type[] argumentTypes) {
    if (enclosingMethodArguments.length != argumentTypes.length) {
      ...
      for (int i = 0; i < argumentTypes.length; i++) {
        if (!enclosingMethodArguments[i].equals(argumentTypes[i])) {
          return false;
        }
      }
    }
